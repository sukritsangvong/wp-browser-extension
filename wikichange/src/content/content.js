...
const renderSlider = (creationDate) => {
  ...
  dateInput.addEventListener("change", function (ev) {
    let daysDiff =
      (new Date(this.value).getTime() - creationDate.getTime()) /
      (1000 * 3600 * 24);
...
const highlightContentWithContext = (json, color) => {
  ...
  while ((foundIndex = innerHTML.indexOf(highlight, controlIndex)) != -1) {
    ...
    let beforeText = innerHTML.substring(
      foundIndex - before.length,
      foundIndex
    );
...
const highlightContentUsingNodes = (context, color) => {
  ...
  textNodes.every((textNode) => {
    ...
    if (newValue !== value) {
      if (
        value.includes(context.content_after) ||
        value.includes(context.content_before)
      ) {
        ...
        if (
          ...
          (node.parentNode.nextSibling.nodeValue.includes(
            ...
            node.parentNode.previousSibling.nodeValue.includes(
...
(() => {
  let wiki_data_url;
  try {
    wiki_data_url = document
      .getElementById("t-wikibase")
